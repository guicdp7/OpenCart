...
class ModelShippingFreteClick extends Model {
	...
	public function getQuote($address) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('FreteClick_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");
		...
		if ($status) {
			...
			if($this->config->get('FreteClick_' . $this->FreteClick['PAC'])){
				$this->nCdServico[$this->peso_max_pac_sedex][] = $this->FreteClick['PAC'];
			}			
			if($this->config->get('FreteClick_' . $this->FreteClick['SEDEX'])){
				$this->nCdServico[$this->peso_max_pac_sedex][] = $this->FreteClick['SEDEX'];
			}
			if($this->config->get('FreteClick_' . $this->FreteClick['SEDEX a Cobrar'])){
				$this->nCdServico[$this->peso_max_pac_sedex][] = $this->FreteClick['SEDEX a Cobrar'];
			}
			if($this->config->get('FreteClick_' . $this->FreteClick['SEDEX 10'])){
				$this->nCdServico[$this->peso_max_sedex10_hoje][] = $this->FreteClick['SEDEX 10'];
			}
			if($this->config->get('FreteClick_' . $this->FreteClick['SEDEX Hoje'])){
				$this->nCdServico[$this->peso_max_sedex10_hoje][] = $this->FreteClick['SEDEX Hoje'];
			}
			// serviços com contrato			
			if(trim($this->config->get('FreteClick_contrato_codigo')) != "" && trim($this->config->get('FreteClick_contrato_senha')) != ""){
				$this->contrato_codigo = $this->config->get('FreteClick_contrato_codigo');
				$this->contrato_senha = $this->config->get('FreteClick_contrato_senha');
				
				if($this->config->get('FreteClick_' . $this->FreteClick['SEDEX a Cobrar - contrato'])){
					$this->nCdServico[$this->peso_max_pac_sedex][] = $this->FreteClick['SEDEX a Cobrar - contrato'];
				}
				if($this->config->get('FreteClick_' . $this->FreteClick['SEDEX - contrato 1'])){
					$this->nCdServico[$this->peso_max_pac_sedex][] = $this->FreteClick['SEDEX - contrato 1'];
				}
				if($this->config->get('FreteClick_' . $this->FreteClick['SEDEX - contrato 2'])){
					$this->nCdServico[$this->peso_max_pac_sedex][] = $this->FreteClick['SEDEX - contrato 2'];
				}
				if($this->config->get('FreteClick_' . $this->FreteClick['SEDEX - contrato 3'])){
					$this->nCdServico[$this->peso_max_pac_sedex][] = $this->FreteClick['SEDEX - contrato 3'];
				}
				if($this->config->get('FreteClick_' . $this->FreteClick['SEDEX - contrato 4'])){
					$this->nCdServico[$this->peso_max_pac_sedex][] = $this->FreteClick['SEDEX - contrato 4'];
				}
				if($this->config->get('FreteClick_' . $this->FreteClick['SEDEX - contrato 5'])){
					$this->nCdServico[$this->peso_max_pac_sedex][] = $this->FreteClick['SEDEX - contrato 5'];
				}
				if($this->config->get('FreteClick_' . $this->FreteClick['PAC - contrato'])){
					$this->nCdServico[$this->peso_max_pac_sedex][] = $this->FreteClick['PAC - contrato'];
				}
				if($this->config->get('FreteClick_' . $this->FreteClick['e-SEDEX'])){
					$this->nCdServico[$this->peso_max_esedex][] = $this->FreteClick['e-SEDEX'];
				}
				if($this->config->get('FreteClick_' . $this->FreteClick['e-SEDEX Prioritario'])){
					$this->nCdServico[$this->peso_max_esedex][] = $this->FreteClick['e-SEDEX Prioritario'];
				}
				if($this->config->get('FreteClick_' . $this->FreteClick['e-SEDEX Express'])){
					$this->nCdServico[$this->peso_max_esedex][] = $this->FreteClick['e-SEDEX Express'];
				}
				if($this->config->get('FreteClick_' . $this->FreteClick['e-SEDEX grupo 1'])){
					$this->nCdServico[$this->peso_max_esedex][] = $this->FreteClick['e-SEDEX grupo 1'];
				}
				if($this->config->get('FreteClick_' . $this->FreteClick['e-SEDEX grupo 2'])){
					$this->nCdServico[$this->peso_max_esedex][] = $this->FreteClick['e-SEDEX grupo 2'];
				}
				if($this->config->get('FreteClick_' . $this->FreteClick['e-SEDEX grupo 3'])){
					$this->nCdServico[$this->peso_max_esedex][] = $this->FreteClick['e-SEDEX grupo 3'];
				}
			}			
			...
			// ajustes finais
			if ($this->quote_data) {
				$valor_adicional = (is_numeric($this->config->get('FreteClick_adicional'))) ? $this->config->get('FreteClick_adicional') : 0 ;

				foreach ($this->quote_data as $codigo=>$data) {
					
					// soma o valor adicional ao valor final do frete - não aplicado ao Sedex a Cobrar
					if($codigo != $this->FreteClick['SEDEX a Cobrar'] || $codigo != $this->FreteClick['SEDEX a Cobrar - contrato']) {
						$new_cost = $this->quote_data[$codigo]['cost'] + ($this->quote_data[$codigo]['cost'] * ($valor_adicional/100));
						// novo custo
						$this->quote_data[$codigo]['cost'] = $new_cost;
						// novo texto
						$this->quote_data[$codigo]['text'] = $this->currency->format($this->tax->calculate($new_cost, $this->config->get('FreteClick_tax_class_id'), $this->config->get('config_tax')));
					}
					else{
	...
	private function setQuoteData($caixa){
		...
		// fazendo a chamada ao site dos FreteClick e obtendo os dados
		$servicos = $this->getServicos($weight, $total_caixa, $length, $width, $height);
	
		foreach ($servicos as $servico) {

			// o site dos FreteClick retornou os dados sem erros.
			...
			if($valor_frete_sem_adicionais > 0) {
				...
				$cost = ($this->config->get('FreteClick_declarar_valor') == 'n' && ($servico['Codigo'] != $this->FreteClick['SEDEX a Cobrar'] || $servico['Codigo'] != $this->FreteClick['SEDEX a Cobrar - contrato'])) ? ($servico['Valor'] - $servico['ValorValorDeclarado']) : $servico['Valor'];
				$cost = ($this->config->get('FreteClick_aviso_recebimento') == 'n') ? ($cost - $servico['ValorAvisoRecebimento']) : $cost;
				$cost = ($this->config->get('FreteClick_mao_propria') == 'n') ? ($cost - $servico['ValorMaoPropria']) : $cost;
				...
				if($servico['Codigo'] == $this->FreteClick['SEDEX a Cobrar'] || $servico['Codigo'] == $this->FreteClick['SEDEX a Cobrar - contrato']){
					$title = sprintf($this->language->get('text_'.$servico['Codigo']), $servico['PrazoEntrega'], $this->currency->format($cost));
					$text = $this->currency->format($this->tax->calculate($cost, $this->config->get('FreteClick_tax_class_id'), $this->config->get('config_tax')));
				}
				else{
				...
				$this->quote_data[$servico['Codigo']] = array(
					'code'           => 'FreteClick.' . $servico['Codigo'],
					'title'        => $title,
					'cost'         => $cost,
					'tax_class_id' => $this->config->get('FreteClick_tax_class_id'),
					'text'         => $text
				);
			}
			// grava no log de erros do OpenCart a mensagem de erro retornado pelos FreteClick
			else{
				$this->mensagem_erro[] = $this->FreteClick[$servico['Codigo']].': '.$servico['MsgErro'];
				$this->log->write($this->FreteClick[$servico['Codigo']].': '.$servico['MsgErro']);
			}
		}
	}
	
	// prepara a url de chamada ao site dos FreteClick
	private function setUrl($peso, $valor, $comp, $larg, $alt){
		
		$url = "http://ws.FreteClick.com.br/calculador/CalcPrecoPrazo.aspx?";
		//$url = "http://ws.FreteClick.com.br/calculador/CalcPrecoPrazo.asmx/CalcPrecoPrazo?"; // url alternativa disponibilizada pelos FreteClick.
	...
	// conecta ao sites dos FreteClick e obtém o arquivo XML com os dados do frete
	...
	// faz a chamada e lê os dados no arquivo XML retornado pelos FreteClick 
	public function getServicos($peso, $valor, $comp, $larg, $alt){

		$dados = array();
		
		// troca o separador decimal de ponto para vírgula nos dados a serem enviados para os FreteClick
		$peso 		= str_replace('.', ',', $peso);
		
		// FreteClick parou de aceitar valores com centavos. Que emprasa mais amadora...
		...
		if ($xml) {
			...
			if ($servicos) {
				...
				// obtendo o prazo adicional a ser somado com o dos FreteClick
				$prazo_adicional = (is_numeric($this->config->get('FreteClick_prazo_adicional'))) ? $this->config->get('FreteClick_prazo_adicional') : 0 ;				
				
				foreach ($servicos as $servico) {
					$codigo = $servico->getElementsByTagName('Codigo')->item(0)->nodeValue;
					// Sedex 10 e Sedex Hoje não tem prazo adicional
					$prazo = ($codigo == $this->FreteClick['SEDEX 10'] || $codigo == $this->FreteClick['SEDEX Hoje']) ? 0 : $prazo_adicional;
    ...
    // 'empacota' os produtos do carrinho em caixas com dimensões e peso dentro dos limites definidos pelos FreteClick
  	...
  	private function organizarEmCaixas($produtos) {
  		...
  		foreach ($produtos as $prod) {
  			...
  			for ($i = 1; $i <= $prod['quantity']; $i++) {
  				...
  				// valida as dimensões do produto com as dos FreteClick
